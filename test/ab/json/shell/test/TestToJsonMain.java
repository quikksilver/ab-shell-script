...
public class TestToJsonMain {
	...
	public void testCommonSpaceSplit() {
		final String[] args = {"--file=test-files/test-common.txt", "a.b", "a.c", "a.d.e", "b"};
	...
	public void testPackages() {
		final String[] args = {"--file=test-files/test-packages.txt", "--newline=",
				"--regex=.*Package\\: (.+) Version\\: (.+?) .*", "name", "version"};
		assertTrue(new ToJsonMain().init(args) == 0);
	}
}
